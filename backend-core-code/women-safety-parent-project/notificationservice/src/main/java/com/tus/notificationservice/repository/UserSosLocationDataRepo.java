...
public interface UserSosLocationDataRepo extends JpaRepository<UserSosLocationData, UserSosLocationData.SosUserId> {
    ...
    @Query(value = "SELECT * FROM user_sos_location_data u WHERE u.sos_id = :sos_id", nativeQuery = true)
    public ArrayList<UserSosLocationData> getSosNotificationBySosId(@Param("sos_id") Long sos_id);
